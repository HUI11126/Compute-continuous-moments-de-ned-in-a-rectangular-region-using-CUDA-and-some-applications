...
{  
  ...
  // SAFE_CALL(cudaMemcpy(lambda_cpu, lambda_coff, ORD * ORD * sizeof(float), cudaMemcpyDeviceToHost), "lambda to lambda_cpu failed");
  // for(int i=768; i<800; i++)
  // {
  //   for(int j=768; j<800; j++) 
  //   { 
  ...
  cv::imwrite("test_images/lenna-1024_" + std::to_string(K) +"_"+ std::to_string(ORD) + "_recons.jpg", recon_img_cpu);
