...
#define K 3
#define ORD1 96
#define ORD2 640  // Legendre polynomial的阶数order
...
__constant__ float dj[ORD2];
__constant__ float j2_1[ORD2]; //2 * j -1
__constant__ float j_1[ORD2]; // j - 1
...
__global__ void p_polynomial(float *v, const int W, const float div, const float div1)
{
  ...
  if(xIdx < W)
  {
    ...
    float p0 = 1.0f;
    float p1 = temp_x;
    for(int j=2; j<ORD2; j++)
    { 
      float temp_v = (j2_1[j] * temp_x * p1 - j_1[j] * p0) * dj[j];
      p0 = p1;
      p1 = temp_v;

      if(j >= ORD1)
        v[xIdx + (j-ORD1) * W] = temp_v;
    }
  }
}
...
__global__ void multiply_coff(float *lambda_coff, float *lambda, int imgHeight_k, int imgWidth_k)
{
  ...
  // double
  // if(Row+Col <= (ORD2-ORD1))
  if(Row+Col < (ORD2-ORD1))
    lambda_coff[Row*(ORD2-ORD1) + Col] = lambda[Row*(ORD2-ORD1) + Col] * (float)((2*(Row+ORD1) + 1) * (2*(Col+ORD1) + 1)) / (float)(imgHeight_k * imgWidth_k);
  else
    lambda_coff[Row*(ORD2-ORD1) + Col] = 0.f;
}
...
__global__ void recon_img_f2u(uchar *recon_img, float *recon_img_float, int imgHeight, int imgWidth)
{
  ...
  {
    // recon_img[Row*imgWidth + Col] = ((recon_img_float[Row*imgWidth + Col] + 1.f) * 127.5f - 120.f) * 500.f;
...
{  
  ...
  SAFE_CALL(cudaMalloc((void**)&p_in_Xdir_k, (ORD2-ORD1) * imgWidth_k * sizeof(float)), "cudaMalloc p_in_Xdir_k failed");  // 计算image moments
  SAFE_CALL(cudaMalloc((void**)&p_in_Ydir_k, (ORD2-ORD1) * imgHeight_k * sizeof(float)), "cudaMalloc p_in_Ydir_k failed");  
  SAFE_CALL(cudaMalloc((void**)&p_in_Xdir, (ORD2-ORD1) * imgWidth * sizeof(float)), "cudaMalloc p_in_Xdir failed");    // 重建图像
  SAFE_CALL(cudaMalloc((void**)&p_in_Ydir, (ORD2-ORD1) * imgHeight * sizeof(float)), "cudaMalloc p_in_Ydir failed"); 
  ...
  float *temp_dj = new float[ORD2];
  temp_dj[0] = 1.0f;
  temp_dj[1] = 1.0f;
  float *temp_j2_1 = new float[ORD2];
  temp_j2_1[0] = 1.0f;
  temp_j2_1[1] = 1.0f;
  float *temp_j_1 = new float[ORD2];
  temp_j_1[0] = 1.0f;
  temp_j_1[1] = 1.0f;
  for(int j=2; j<ORD2; j++)
  {
  ...
  SAFE_CALL(cudaMemcpyToSymbol(dj, temp_dj, ORD2 * sizeof(float)), "cudaMemcpyToSymbol dj failed");
  SAFE_CALL(cudaMemcpyToSymbol(j2_1, temp_j2_1, ORD2 * sizeof(float)), "cudaMemcpyToSymbol j2_1 failed");
  SAFE_CALL(cudaMemcpyToSymbol(j_1, temp_j_1, ORD2 * sizeof(float)), "cudaMemcpyToSymbol j_1 failed");
  ...
  SAFE_CALL(cudaMalloc((void**)&p_in_Xdir_trans_k, (ORD2-ORD1) * imgWidth_k * sizeof(float)), "cudaMalloc p_in_Xdir_trans_k failed");   
  dim3 gridDim_trans_k((imgWidth_k + blockDim_trans.x - 1) / blockDim_trans.x, ((ORD2-ORD1) + blockDim_trans.y - 1) / blockDim_trans.y);
  matrix_transpose<<<gridDim_trans_k, blockDim_trans>>>(p_in_Xdir_trans_k, p_in_Xdir_k, imgWidth_k, ORD2-ORD1);
  ...
  float *AC;
  SAFE_CALL(cudaMalloc((void**)&AC, imgHeight_k * (ORD2-ORD1) * sizeof(float)), "cudaMalloc AC failed");
  const float a = 1.f, b = 0.f;
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, (ORD2-ORD1), imgHeight_k, imgWidth_k, &a, 
              p_in_Xdir_trans_k, (ORD2-ORD1), resImg, imgWidth_k, &b, AC, (ORD2-ORD1));
  ...
  float *lambda;
  float *lambda_cpu = new float[(ORD2-ORD1) * (ORD2-ORD1)];
  SAFE_CALL(cudaMalloc((void**)&lambda, (ORD2-ORD1) * (ORD2-ORD1) * sizeof(float)), "cudaMalloc lambda failed");
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, (ORD2-ORD1), (ORD2-ORD1), imgHeight_k, &a, 
              AC, (ORD2-ORD1), p_in_Ydir_k, imgHeight_k, &b, lambda, (ORD2-ORD1));

  //////////////////////////////////////////////////// 重建图像 /////////////////////////////////////////////////////
  float *p_in_Ydir_trans;
  SAFE_CALL(cudaMalloc((void**)&p_in_Ydir_trans, (ORD2-ORD1) * imgHeight * sizeof(float)), "cudaMalloc p_in_Ydir_trans failed"); 
  dim3 gridDim_trans((imgHeight + blockDim_trans.x - 1) / blockDim_trans.x, ((ORD2-ORD1) + blockDim_trans.y - 1) / blockDim_trans.y);
  matrix_transpose<<<gridDim_trans, blockDim_trans>>>(p_in_Ydir_trans, p_in_Ydir, imgHeight, (ORD2-ORD1));

  float *lambda_coff;
  SAFE_CALL(cudaMalloc((void**)&lambda_coff, (ORD2-ORD1) * (ORD2-ORD1) * sizeof(float)), "cudaMalloc lambda_coff failed");
  dim3 blockDim_coff(32, 32);
  dim3 gridDim_coff(((ORD2-ORD1) + blockDim_coff.x-1) / blockDim_coff.x, ((ORD2-ORD1) + blockDim_coff.y-1) / blockDim_coff.y);
  multiply_coff<<<gridDim_coff, blockDim_coff>>>(lambda_coff, lambda, imgHeight_k, imgWidth_k);

  // SAFE_CALL(cudaMemcpy(lambda_cpu, lambda_coff, (ORD2-ORD1) * (ORD2-ORD1) * sizeof(float), cudaMemcpyDeviceToHost), "lambda to lambda_cpu failed");
  // for(int i=0; i<(ORD2-ORD1); i++)
  // {
  //   for(int j=0; j<(ORD2-ORD1); j++) 
  //   { 
  //     std::cout << lambda_cpu[i*(ORD2-ORD1) + j] <<" ";
  //   }
  //   std::cout << std::endl;
  // }
  ...
  SAFE_CALL(cudaMalloc((void**)&recon_img_tmp, (ORD2-ORD1) * imgWidth * sizeof(float)), "cudaMalloc recon_img_tmp failed");
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, imgWidth, (ORD2-ORD1), (ORD2-ORD1), &a, 
              p_in_Xdir, imgWidth, lambda_coff, (ORD2-ORD1), &b, recon_img_tmp, imgWidth);
  cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, imgWidth, imgHeight, (ORD2-ORD1), &a,
              recon_img_tmp, imgWidth, p_in_Ydir_trans, (ORD2-ORD1), &b, recon_img_float, imgWidth);
  ...
  cv::imwrite("test_images/lenna-1024_" + std::to_string(K) 
              + "_"+ std::to_string(ORD1) +"_" + std::to_string(ORD2) + "_edge.jpg", recon_img_cpu);
